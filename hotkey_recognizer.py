...
import time
import uuid
class Hotkey_Recognizer(sr.Recognizer):
    def listen(self, source, end_listen_key, timeout=None, phrase_time_limit=None, snowboy_configuration=None):
        """
        ...
        while True:
            ...
            while True:# check if hotkey is still pressed
                ...
                if len(buffer) == 0: break  # reached end of the stream
                frames.append(buffer)
                phrase_count += 1# check if hotkey is still pressed
                ...
                if(keyboard.is_pressed(end_listen_key) and elapsed_time>0.5):
                    break
